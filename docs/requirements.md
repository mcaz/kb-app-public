...
- **FR-C8 添付(2026-08-10 本人決定・実装済み)**: ノートは画像・原本ファイル等を
  「所有」できる。実体は **同名サイドカー `<id>.files/`**(C案)— ノート=1ファイルの
  OKF 互換を壊さず(`index.md` 予約名衝突の A 案・ID が汚れる B 案は棄却)、
  「1ノート=1単位」の対の管理(rename/move/archive 時の同伴)はアプリが保証する。
  UI 語彙は「添付」(ディレクトリという言葉は見せない — 原則7)。
  サイズガード: 10MB 警告 / 50MB 拒否(GitHub 同期の保全)。添付ファイル名は検索対象。
  出典にする添付は OKF `sources[].resource` で `/…files/…` を指せる(標準語彙の provenance)。
  画像はエディタへのペーストで自動添付+リンク挿入、プレビューで表示。
  添付の中身検索(PDF 抽出等)と「同期に乗せないローカル添付」は将来
