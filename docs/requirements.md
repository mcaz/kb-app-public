...
8. **維持作業はアプリが回す**。整理・重複・リンク・鮮度といった知識の手入れをユーザーに
   ...
   「はい / いいえ」で答えるだけ
9. **手書きのノートは聖域**。ユーザーが自分の言葉で書いたメモと、AI による自己編集は相性が
   悪い(2026-08-09 本人指摘)。AI・お手入れがユーザーの書いた本文を**書き換える・統合する
   提案は既定でしない**。手書きノートに対して許されるのは非破壊の3種のみ —
   ①新しい下書きの追加提案 ②つながりの提案 ③気づきの表示(重複・古さを「知らせる」だけ)。
   書き換え型の手入れは、AI 由来ノート同士か、本人が明示的に頼んだときに限る
...
- ノートの書き手区別(手書き / AI 由来)の持ち方 — 原則9 の判定に必要。ユーザーに見せる概念には
  しない(NFR-6)
