...
- **FR-C1 ストア**: vault = 1ディレクトリ(内部は git リポ)。ノートは Markdown +最小
  ...
  - **形式は OKF(Open Knowledge Format)互換を基本方針とする(2026-08-09 決定)**:
    ...
    パス=ID・type 必須+推奨5フィールドの最小規約)。互換にすることで「ロックインしない」が
    標準準拠という外部証明になり、OKF ツール群とバンドルを相互運用できる。app 固有の
    メタ(status: draft、書き手区別=メモ/育つノート)は拡張フィールドとして分離して乗せる。
    spec は v0.1 と若いためハードロックせず追随余地を残す(精読と適合設計は技術設計の最初の
    タスク。https://github.com/GoogleCloudPlatform/knowledge-catalog/tree/main/okf)
...
- frontmatter の詳細設計 — OKF v0.1 準拠+app 拡張(status・書き手区別)の適合方法。
  spec 精読が技術設計の最初のタスク
