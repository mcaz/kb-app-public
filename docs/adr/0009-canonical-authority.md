...
2026-09-08追記（[Issue #93](https://github.com/mcaz/kb-app/issues/93)）:
「完了したか」「以前の結果は何か」という質問では、現行の一般方針が正確でも、それだけでは
対象の実行結果に答えられない。対象に一致し、本文に結果の根拠がある候補を優先する検索信号を加える。
`historical`だけを一律に昇格する案は、別対象の過去記録や結果のない旧計画も上がるため採らない。
完了・成功だけでなく、未完了・失敗・未確認の記録も質問への結果となる。

この信号は検索用途に限り、本文からauthorityを推定・書換したり、検索上位を現行方針に昇格したり
しない。完全タイトル一致、現行方針への明示的な質問、意図なしqueryの既定順を維持し、完了条件や
分類方法の質問を結果確認へ広げない。候補抽出と融合後の最終順位で同じ判断を使い、DB schema・
index・公開APIの追加を前提にしない。ContextCardのrerank・本文予算の変更とは独立した検索側の修正とする。
再現・対照と測定の区別は[完了・過去結果の検索](../retrieval-completion-results.md)に記録する。
