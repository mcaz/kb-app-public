...
### 13. 表示中の情報を定期取得し、アプリ復帰でも更新する(2026-09-06 本人決定)

ホームや提案の手動更新を減らすため、本人は「数秒〜15秒で反映し、アプリへ戻ったら
すぐ更新」を選択した。`liveQueryOptions` に15秒の再取得と復帰時の対象指定を集約する。
ホーム、観測、ノート、提案、検索、タグ、グラフ、ファイル一覧と設定の読み取りへ適用する。
ローカルのTauri IPCはオフラインでも動くので、ブラウザの接続状態で取得を止めない。
非表示時・購読の無い画面・無効なqueryは定期取得しない。

復帰時は共通hookがDOMのfocus/visibilitychangeとTauriのnative focusを受け、
購読中の対象だけを再取得する。Webviewのvisibilityが変わらないアプリ切替も拾い、
同時に届く通知はまとめる。進行中の取得を中断して同じ処理を再発行しない。
取得中に届いた復帰・変更検知はqueryごとに予約し、先行取得の完了後に1回読み直す。
変更前に始めた応答へ要求を吸収させない。画面を離れた場合は予約も停止する。
認証・Keychain・プレビュー等を一律に再取得する設定にはしない。

ノート本文の`note_get`は読取専用にする。AI向けの現在ノート記録は主ノートを開いた時の
`note_set_current`へ分離し、プレビューや再取得で文脈を上書きしない。
現在ノートの記録とAI起動は同じmutation scopeで直列化する。
閉じた検索・関連モーダルでは購読を無効にし、入力中の検索条件は保持する。
提案の採否フォームはノートUID単位で入力を保ち、更新前の確認だけを破棄して再確認を求める。

同期・export・索引保守・ノート数履歴の記録は、表示の読み取りと分けて60秒間隔で実行する。
復帰時も保守の60秒の鮮度を守る。別端末の変更はpull後、履歴は記録後に表示へ反映する。
これは本節以前の「索引syncは一覧取得で実行する」という記述を置き換える現行方針であり、
前景の読み取りは共有DBからの取得に留める。

MCPはGUIとは別プロセスなので、GUIの操作だけにイベントを足しても外部の保存は拾えない。
2026-09-08の表示遅延調査で、遅い旧応答の完了後にさらに15秒待つ経路を確認した。
共通hookは前面表示中に1秒ごとに`note_revision`を確認する。既存の`cadence_cache`が持つ
DB generationとnotes revisionを読み、初回・変化時だけlive queryを無効化する。
未購読の画面は再表示時に取得し、購読中の画面は上記の予約処理で再取得する。
トリガーがノート・関連の変更と同じtransactionでrevisionを進めるため、旧MCPも対象となり、
rollbackは変更と数えない。修復でrevisionが巻き戻ってもgenerationで識別する。

1秒は軽量確認の間隔であり、取得処理・DBロック待ちを含む表示時間の保証上限ではない。
検知失敗時はホームに案内を出し、15秒の通常取得を続ける。ホームは取得中・最終成功時刻と
更新ボタンを示し、再取得失敗でキャッシュを残す場合は前回の値であることを明示する。
最終取得時刻は応答が成功した時刻であり、保存時刻や描画完了の計測値ではない。
Web Workerやソケット、保存側への通知処理は追加しない。
