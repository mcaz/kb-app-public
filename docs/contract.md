...
| 4 | **同期・検索索引は派生**。壊れたら画面に出す(沈黙しない) | 続行可能な失敗は `data + Degradation[]` で返し、安定した `code` をUI/MCPへ表示 / 取得失敗を空配列へ変換して「0件」と偽らない / Markdown出力失敗は`markdown_export`として残す / 続行不能だけをerrorにする |
...
- 契約8の自動retrievalはserver instructionsによる自発的なtool選択とは別に実行する。hookは
  ...
  OSガードを再判定してから、公開されたsearchのDB本文同梱オプションだけを呼ぶ。Claudeだけにあった旧Python
