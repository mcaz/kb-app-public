...
- 起動時のschema不整合を空の保管庫として扱わない。schema10より古い宣言にschema10の蒸留台帳が
  残っているDBはjournal mode変更・migration前に拒否する。診断用の`inspect_runtime_storage`は
...
- 現存Markdownの限定復旧は、本人が起動する`--storage-recovery`専用画面だけに公開する。
  ...
  既存のexport・蒸留・action台帳は全行hashで保持を確認する。原文・UID・authority・参照関係と
...
- 契約8の接続先照合は、GUIでの完全保護設定・Claude Desktop接続時に固定した
